...
func main() {
	...
	// ── Transfer via the unified entry (wallet is empty; this exercises the
	// endpoint/validation path). CreateTransfer is the pre-1.8 wrapper around
	// CreateTransaction(&paratro.TransferRequest{...}); both POST /api/v1/transactions.
	step("CreateTransfer", func() (interface{}, error) {
		return client.Transaction.CreateTransfer(ctx, &paratro.TransferRequest{
			...
			ReferenceID: fmt.Sprintf("sdk-smoke-%d", time.Now().UnixNano()),
		})
	})

	// ── x402 facilitator (payloads are intentionally empty: validation path only) ──
	step("X402ListSettlements", func() (interface{}, error) {
		return client.X402.X402ListSettlements(ctx, &paratro.ListX402SettlementsRequest{Page: 1, PageSize: 10})
	})
	step("X402Verify", func() (interface{}, error) {
		return client.X402.X402Verify(ctx, map[string]interface{}{"x402Version": 2})
	})
