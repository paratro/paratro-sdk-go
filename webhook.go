...
// Webhook event types emitted by Paratro.
const (
	// EventTransactionConfirming: an inbound deposit was seen on chain and is
	// accumulating confirmations.
	EventTransactionConfirming = "transaction.confirming"
	// EventTransactionConfirmed: a deposit or an outbound transaction (TRANSFER,
	// PROGRAM_CALL, CONTRACT_CALL) reached the required confirmations.
	EventTransactionConfirmed = "transaction.confirmed"
	// EventTransactionFailed: an outbound transaction failed on chain.
	EventTransactionFailed = "transaction.failed"
	// EventTransferCredited: a payment from another Paratro-hosted wallet was
	// credited to your account (transaction_type "INTERNAL"). Deliberately not
	// "transaction.confirmed" so deposit handlers do not pick it up unasked.
	EventTransferCredited = "transfer.credited"
	// EventX402SettlementConfirmed: an x402 settlement was credited to the
	// receiving (seller) account.
