...
# 2. Create wallet
curl -s -X POST https://api.example.com/api/v1/wallets \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"wallet_name":"My Wallet"}'
...
curl -s -X POST https://api.example.com/api/v1/assets \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"account_id":"<account_id>","symbol":"USDT","chain":"ethereum"}'

# 5. Create transfer
curl -s -X POST https://api.example.com/api/v1/transfer \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"from_address":"<from_address>","to_address":"0xbbbb...","chain":"ethereum","token_symbol":"USDT","amount":"10.5"}'

# 6. List transactions
curl -s "https://api.example.com/api/v1/transactions?wallet_id=<wallet_id>&page=1&page_size=20" \
  -H "Authorization: Bearer $TOKEN"
```
